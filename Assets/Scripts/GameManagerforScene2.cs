...
public class GameManagerforScene2 : MonoBehaviour {
	...
	public static GameManagerforScene2 gm;
	...
	public GameObject restartTheGameButtons;
	public string restartTheGame;
	...
	void Start () {
		...
		if (gm == null) 
			gm = this.gameObject.GetComponent<GameManagerforScene2>();
		...
		if (restartTheGameButtons)
			restartTheGameButtons.SetActive (false);
	...
	void Update () {
		if (!gameIsOver) {
			if (canBeatLevel && (score >= beatLevelScore)) {  // check to see if beat game
				...
				mainTimerDisplay.text = currentTime.ToString ("0.00");		// To format the time with 0.00 precision		
			}
		}
	}
	...
	void EndGame() {
		...
		if (restartTheGameButtons)
			restartTheGameButtons.SetActive (true);
	...
	public void RestartGame ()
	{
		...
		Application.LoadLevel (restartTheGame);
	}
